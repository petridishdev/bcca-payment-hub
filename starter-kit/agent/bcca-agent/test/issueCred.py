...
DEFAULT_AGENT_URL = os.environ.get('AGENT_URL', 'http://localhost:5001/bcca')
    ...
    async with aiohttp.ClientSession() as http_client:
        ...
        for cred_path in cred_paths:
            if os.path.isfile(cred_path):
                cred_files = [cred_path,]
            else:
                cred_files = glob.glob(cred_path)
            for cred_file in cred_files:
                req = issue_cred(http_client, cred_file, idx)
                if parallel:
                    all.append(req)
                else:
                    await req
